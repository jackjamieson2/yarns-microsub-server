...
// Post Content Filter
add_filter( 'microsub_post_content', array( 'Microsub_Render', 'generate_post_content' ), 1, 2 );


/**
 * Microsub Render Class
 */
class Microsub_Render {
	...
	public static function generate_post_content( $post_content, $input ) {
		...
		foreach ( array_keys( $verbs ) as $prop ) {
			...
			if ( $val ) {
				...
				// https://microsub.net/draft/#nested-microformats-objects
				if ( is_string( $val ) ) {
					$val = array(
						'url' => $val,
					);
		...
		// event
		if ( array( 'h-event' ) === ms_get( $input, 'type' ) ) {
