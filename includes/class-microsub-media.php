...
add_action( 'plugins_loaded', array( 'Microsub_Media', 'init' ) );

/**
 * Microsub Media Class
 */
class Microsub_Media {
	...
	public static function init() {
		...
		add_action( 'rest_api_init', array( $cls, 'register_route' ) );
				...
				// endpoint discovery
				add_action( 'wp_head', array( $cls, 'microsub_media_html_header' ), 99 );
				add_action( 'send_headers', array( $cls, 'microsub_media_http_header' ) );
				add_filter( 'host_meta', array( $cls, 'microsub_media_jrd_links' ) );
				add_filter( 'webfinger_user_data', array( $cls, 'microsub_media_jrd_links' ) );

	}

	public static function get_microsub_media_endpoint() {
			return rest_url( MICROSUB_NAMESPACE . '/media' );
	}

		/**
		 * The microsub autodicovery meta tags
		 */
	public static function microsub_media_html_header() {
			// phpcs:ignore
			printf( '<link rel="microsub_media" href="%s" />' . PHP_EOL, static::get_microsub_media_endpoint() );
	}

		/**
		 * The microsub autodicovery http-header
		 */
	public static function microsub_media_http_header() {
			Microsub_Endpoint::header( 'Link', '<' . static::get_microsub_media_endpoint() . '>; rel="microsub_media"' );
	}

		/**
		 * Generates webfinger/host-meta links
		 */
	public static function microsub_media_jrd_links( $array ) {
			$array['links'][] = array(
				'rel'  => 'microsub_media',
				'href' => static::get_microsub_media_endpoint(),
			);
			return $array;
	}
	...
	public static function upload_from_file( $files, $name = null ) {
		...
		if ( isset( $file['error'] ) ) {
			return new WP_Microsub_Error( 'invalid_request', $file['error'], 500 );
		}
	...
	public static function upload_from_url( $url ) {
		if ( ! wp_http_validate_url( $url ) ) {
			return new WP_Microsub_Error( 'invalid_request', 'Invalid Media URL', 400 );
		}
		require_once ABSPATH . 'wp-admin/includes/file.php';
		$tmp = download_url( $url );
		if ( is_wp_error( $tmp ) ) {
			return new WP_Microsub_Error( 'invalid_request', $tmp->get_message(), 400 );
		}
		$file_array = array(
			'name'     => basename( $url ),
			'tmp_name' => $tmp,
	 ...
	 * @return WP_Microsub_Error|true Boolean true if the attachment may be created, or a WP_Microsub_Error if not.
	 */
	protected static function permissions_check( $request ) {
		...
		if ( 'POST' === $request->get_method() ) {
			if ( ! current_user_can( 'upload_files' ) ) {
				return new WP_Microsub_Error( 'forbidden', 'User is not permitted to upload files', 403 );
			}
		}
		$intersect = array_intersect( array( 'create', 'media' ), static::$scopes );
		if ( empty( $intersect ) ) {
			return new WP_Microsub_Error( 'insufficient_scope', 'You do not have permission to create or upload media', 401 );
		}
	...
	protected static function insert_attachment( $file, $post_id = 0, $title = null ) {
		...
		if ( is_wp_error( $id ) ) {
			if ( 'db_update_error' === $id->get_error_code() ) {
				return new WP_Microsub_Error( 'invalid_request', 'Database Error On Upload', 500 );
			} else {
				return new WP_Microsub_Error( 'invalid_request', $id->get_error_message(), 400 );
			}
		}
		...
		// Add metadata tag to microsub uploaded so they can be queried
		update_post_meta( $id, '_microsub_upload', 1 );
	...
	public static function upload_handler( $request ) {
		...
		if ( empty( $files ) ) {
			return new WP_Microsub_Error( 'invalid_request', 'No Files Attached', 400 );
		} else {
			$file = self::upload_from_file( $files, 'file' );
		}

		if ( is_microsub_error( $file ) ) {
			return $file;
		}
		...
		add_post_meta( $id, 'microsub_auth_response', static::$microsub_auth_response );
		$data['url'] = $url;
		$data['id']  = $id;
	...
	public static function query_handler( $request ) {
			...
			switch ( $params['q'] ) {
				...
				case 'source':
					if ( array_key_exists( 'url', $params ) ) {
						...
						if ( ! $attachment_id ) {
							return new WP_Microsub_Error( 'invalid_request', sprintf( 'not found: %1$s', $params['url'] ), 400 );
						}
						$resp = self::return_media_data( $attachment_id );
					} else {
						$numberposts = ms_get( $params, 'limit', 10 );
						$attachments = get_posts(
							array(
		...
		return new WP_Microsub_Error( 'invalid_request', 'unknown query', 400, $request->get_query_params() );
	}
